...
dat <- readxl::read_xlsx(path ="files/data/1101_pre_class001.xlsx") %>% drop_na(`時間戳記`)
names(dat) <- c("date","info","device","github","comment")
```
...
```{r plot-device, echo=FALSE, message=FALSE, warning=FALSE,fig.cap="課前個人設備調查"}
ggplot(data=dat,aes(device,fill=device)) +
  geom_bar()
```
調查至`r tail(dat$date,1)`，共有`r dim(dat)[1]`位同學回覆，`r sum(dat$device %in% c("Chromebook","未有個人電腦"))`位同學使用Chromebook或沒有自己的設備。
